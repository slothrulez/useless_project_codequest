      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-stone-800 pb-3 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-emerald-950/80 border-2 border-emerald-500 flex items-center justify-center text-emerald-400">
              <GitCommit className="w-6 h-6" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xs sm:text-sm font-pixel text-emerald-300">COMMIT STATION</h2>
                <span className="px-2 py-0.5 bg-emerald-950 border border-emerald-700 text-emerald-300 font-mono text-[10px] rounded">
                  Branch: {branch}
                </span>
              </div>
              <p className="text-[10px] font-retro text-stone-400">git add . && git commit -m "[message]"</p>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        {isClean && fileList.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs font-retro text-stone-400 max-w-sm mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="bg-stone-950 border border-stone-800 rounded-lg p-2 max-h-48 overflow-y-auto space-y-1.5">
                {fileList.map((file) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-2 rounded cursor-pointer text-xs font-mono transition border ${
                      selectedFile?.path === file.path
                        ? 'bg-stone-800 border-emerald-500 text-emerald-300'
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      <button
                        ...
                        className={`px-1.5 py-0.5 rounded text-[9px] font-pixel transition ${
                          file.status === 'staged'
            ...
            <div className="space-y-2 flex flex-col">
              <div className="font-pixel text-[9px] text-stone-400 flex items-center justify-between">
                ...
                <span className="text-stone-500 font-mono text-[9px] truncate max-w-[150px]">
                  {selectedFile ? selectedFile.path : 'None selected'}
                </span>
              </div>
              <div className="flex-1 bg-stone-950 border border-stone-800 rounded-lg p-3 font-mono text-[11px] overflow-auto max-h-48 text-stone-300">
                {selectedFile ? (
        ...
        {/* Feedback Messages */}
        {errorMsg && (
          <div className="mb-3 p-2.5 rounded bg-red-950/70 border border-red-700 text-xs font-mono text-red-200 flex items-center gap-2">
        ...
        {successMsg && (
          <div className="mb-3 p-2.5 rounded bg-emerald-950/80 border border-emerald-500 text-xs font-mono text-emerald-200 flex items-center gap-2 animate-pulse">
        ...
        <form onSubmit={handleCommit} className="space-y-3 border-t border-stone-800 pt-3">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-stone-950 border border-stone-700 focus:border-emerald-500 rounded-lg px-3 py-2 text-xs font-mono text-stone-100 placeholder-stone-600 focus:outline-none focus:ring-1 focus:ring-emerald-500 transition"
              />
              <span className="absolute right-2.5 top-2.5 text-[10px] font-mono text-stone-600">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {quickMessages.map((msg) => (
              <button
                ...
                className="text-[9px] font-mono bg-stone-800 hover:bg-stone-700 hover:text-emerald-300 text-stone-400 px-2 py-0.5 rounded border border-stone-700 transition"
              >
                {msg.slice(0, 32)}...
              </button>
            ))}
          ...
          <div className="flex items-center justify-between pt-2">
            <span className="text-[10px] font-retro text-stone-500">
              {fileList.length > 0
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1.5 bg-stone-800 hover:bg-stone-700 text-stone-400 hover:text-stone-200 text-xs font-mono rounded transition"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-500 disabled:bg-stone-800 disabled:text-stone-600 text-stone-950 font-pixel text-[9px] font-bold rounded shadow-lg transition flex items-center gap-1.5"
              >
